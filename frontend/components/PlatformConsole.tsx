      ...
      <div class="settings-panel-body">
        <div class="credential-grid">
          <div class="credential-card">
            ...
            <div class="form-group">
              <label class="form-label">OpenAI API key</label>
              <input class="form-input" type="password" id="s-openai" aria-label="OpenAI API key" placeholder="OpenAI API key · sk-...">
            </div>
          </div>
          <div class="credential-card">
            <div class="credential-card-top">
            ...
            <div class="form-group">
              <label class="form-label">Anthropic API key</label>
              <input class="form-input" type="password" id="s-anthropic" aria-label="Anthropic API key" placeholder="Anthropic API key · sk-ant-...">
            </div>
          </div>
          <div class="credential-card credential-card-wide">
            <div class="credential-card-top">
              <span class="badge b-purple">Grok</span>
            </div>
            <div class="credential-card-fields">
              <div class="form-group">
                <label class="form-label">Grok API key</label>
                <input class="form-input" type="password" id="s-xai" aria-label="Grok API key" placeholder="Grok API key · xai-...">
              </div>
              <div class="form-group">
                <label class="form-label">Grok base URL</label>
                <input class="form-input" id="s-xai-base" aria-label="Grok base URL" value="https://api.x.ai/v1">
              </div>
            </div>
          </div>
        </div>
    ...
    <section class="panel settings-models">
      <div class="panel-hd">
        <div class="settings-panel-heading">
          ...
          <span>Select the models agents may use by provider.</span>
        </div>
        <span class="badge b-blue">11 enabled</span>
      </div>
      <div class="settings-panel-body">
        <div class="model-defaults-row">
          ...
          <div class="form-group">
            <label class="form-label">Default Model</label>
            <select class="form-input form-select" id="s-default-model">
              <optgroup label="OpenAI" data-provider="OpenAI"><option>gpt-5.4-nano</option><option>gpt-5-nano</option><option>gpt-5.4-mini</option><option>gpt-5-mini</option><option>gpt-5.5</option><option>gpt-5.4</option></optgroup>
              <optgroup label="Anthropic" data-provider="Anthropic"><option>claude-3-5-haiku-20241022</option><option>claude-3-haiku-20240307</option><option>claude-sonnet-4-20250514</option><option>claude-3-7-sonnet-20250219</option><option>claude-opus-4-1-20250805</option><option>claude-opus-4-20250514</option></optgroup>
              <optgroup label="xAI Grok" data-provider="Grok"><option>grok-4.3</option><option>grok-4-fast-non-reasoning</option><option>grok-build-0.1</option><option>grok-4-fast-reasoning</option><option>grok-3-mini</option><option>grok-3-mini-high</option></optgroup>
            </select>
          </div>
        </div>
        <div class="model-list">
          ...
          <div class="model-provider-card">
            <div class="model-provider-title"><span class="badge b-purple">Grok</span><span class="model-count">3 of 6 enabled</span></div>
            <label class="model-check"><span>grok-4.3</span><input type="checkbox" checked></label>
            <label class="model-check"><span>grok-4-fast-non-reasoning</span><input type="checkbox"></label>
            <label class="model-check"><span>grok-build-0.1</span><input type="checkbox" checked></label>
            <label class="model-check"><span>grok-4-fast-reasoning</span><input type="checkbox" checked></label>
            <label class="model-check"><span>grok-3-mini</span><input type="checkbox"></label>
            <label class="model-check"><span>grok-3-mini-high</span><input type="checkbox"></label>
          </div>
        </div>
      </div>
    </section>

    <section class="panel settings-runtime">
      <div class="panel-hd">
        <div class="settings-panel-heading">
          <span class="panel-title">Runtime Storage</span>
          <span>Queue, database, and execution limits.</span>
        </div>
      </div>
      <div class="settings-panel-body">
        <div class="form-group">
          <label class="form-label">Redis URL</label>
          <input class="form-input" id="s-redis" value="redis://localhost:6379">
          <span class="form-hint">Used for queues, streaming state, and run coordination.</span>
        </div>
        <div class="form-group">
          <label class="form-label">PostgreSQL URL</label>
          <input class="form-input" id="s-pg" value="postgresql://yuno:pass@localhost/yuno">
        </div>
...
export default function PlatformConsole({ initialPage = "dashboard" }: { initialPage?: ConsolePage }) {
  ...
  useEffect(() => {
    ...
    const toolsMcpLink = rootRef.current.querySelector<HTMLElement>("#page-tools .panel-link");
    if (toolsMcpLink) {
      toolsMcpLink.textContent = "Configure keys ->";
      toolsMcpLink.setAttribute("onclick", "nav('settings',document.querySelectorAll('.nav-btn')[7])");
    }
    ...
    const providerSelect = settingsPage?.querySelector<HTMLSelectElement>("#s-default-provider");
    const modelSelect = settingsPage?.querySelector<HTMLSelectElement>("#s-default-model");
    const modelOptgroups = Array.from(modelSelect?.querySelectorAll<HTMLOptGroupElement>("optgroup") ?? []);
    const providerModels = modelOptgroups.reduce<Record<string, string[]>>((models, group) => {
      const provider = group.dataset.provider;
      if (provider) models[provider] = Array.from(group.querySelectorAll("option")).map((option) => option.value);
      return models;
    }, {});
    const filterModelsByProvider = () => {
      const provider = providerSelect?.value ?? "";
      const allowedModels = providerModels[provider] ?? [];
      if (!modelSelect || !allowedModels.length) return;
      const currentModel = modelSelect.value;
      modelSelect.innerHTML = allowedModels.map((model) => `<option>${model}</option>`).join("");
      modelSelect.value = allowedModels.includes(currentModel) ? currentModel : allowedModels[0];
    };
    ...
    const applySavedSettings = () => {
      ...
      setFieldValue("s-openai", saved.openaiKey);
      setFieldValue("s-anthropic", saved.anthropicKey);
      setFieldValue("s-xai", saved.grokKey);
      setFieldValue("s-xai-base", saved.grokBaseUrl);
      setFieldValue("s-default-provider", saved.defaultProvider);
      filterModelsByProvider();
      setFieldValue("s-default-model", saved.defaultModel);
      ...
      setFieldValue("s-redis", saved.redisUrl);
    ...
    scriptTag.text = inlineScript
      ...
      .replace(
        "tools:`<button class=\"btn btn-ghost\" onclick=\"nav('mcp',document.querySelectorAll('.nav-btn')[6])\">MCP Servers</button><button class=\"btn btn-primary\" onclick=\"nav('settings',document.querySelectorAll('.nav-btn')[7])\">Configure Keys</button>`,",
        "tools:`<button class=\"btn btn-primary\" onclick=\"nav('settings',document.querySelectorAll('.nav-btn')[7])\">Configure Keys</button>`,",
      )
      .replace(
        "const PAGE_ACTIONS={",
  ...
  const model=document.getElementById('s-default-model')?.value||'';
  const valid={OpenAI:model.startsWith('gpt-'),Anthropic:model.startsWith('claude-'),Grok:model.startsWith('grok-')}[provider];
  if(!valid){notify('Default model must match selected provider',false);return;}
  ...
  localStorage.setItem('nxflow.settings',JSON.stringify({
    openaiKey:document.getElementById('s-openai')?.value||'',
    anthropicKey:document.getElementById('s-anthropic')?.value||'',
    grokKey:document.getElementById('s-xai')?.value||'',
    grokBaseUrl:document.getElementById('s-xai-base')?.value||'',
    ...
    redisUrl:document.getElementById('s-redis')?.value||'',
  ...
  notify('Settings saved ✓')
    ...
    return () => {
      providerSelect?.removeEventListener("change", filterModelsByProvider);
      scriptTag.remove();
