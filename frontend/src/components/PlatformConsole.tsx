...
const consoleHandlerNames = [
  "addAgentToSelectedWorkflow",
  "addWfAgentRow",
  "canvasClick",
  "clearRunStream",
  "closeModal",
...
function scopedConsoleScript(source: string) {
  const exports = consoleHandlerNames.map((name) => `${name}: typeof ${name} === "function" ? ${name} : undefined`).join(",");
  return `(() => {
...
const agentRunModalMarkup = `
<div class="modal-overlay agent-run-overlay" id="agentRunModal">
  <div class="modal agent-run-modal">
    <div class="modal-hd">
      <span class="modal-title" id="agentRunModalTitle">Run Agent</span>
      <button class="modal-close" onclick="closeModal('agentRunModal')">✕</button>
    </div>
    <div class="modal-body">
      <input type="hidden" id="f-agent-run-id">
      <div class="form-group agent-run-input-group">
        <label class="form-label">Input *</label>
        <textarea class="form-input form-textarea" id="f-agent-run-msg" rows="5" placeholder="Enter the task or question for this agent..."></textarea>
      </div>
    </div>
    <div class="modal-ft">
      <button class="btn btn-ghost" onclick="closeModal('agentRunModal')">Cancel</button>
      <button class="btn btn-primary" onclick="submitAgentRun()">Start Run</button>
    </div>
  </div>
</div>`;
    ...
    <section class="panel settings-models">
      ...
      <div class="settings-panel-body">
        <div class="model-list">
          ...
          <div class="model-provider-card">
            <div class="model-provider-title"><span class="badge b-purple">OpenAI</span><span class="model-count">4 of 4 enabled</span></div>
            <label class="model-check"><span>gpt-4o-mini</span><input type="checkbox" checked></label>
            <label class="model-check"><span>gpt-4o</span><input type="checkbox" checked></label>
            <label class="model-check"><span>gpt-4.1-mini</span><input type="checkbox" checked></label>
            <label class="model-check"><span>gpt-4.1</span><input type="checkbox" checked></label>
          </div>
          <div class="model-provider-card">
...
function filterAgents(value){
  agentSearchValue=value||'';
  agentSearchQuery=agentSearchValue.trim().toLowerCase();
  if(agentSearchTimer) clearTimeout(agentSearchTimer);
  agentSearchTimer=setTimeout(()=>{agentSearchTimer=null;renderAgentsGrid();},80);
}
function renderAgentsGrid(){
  const grid=document.getElementById('agents-grid');
  const search=document.getElementById('agent-search');
  if(!grid)return;
  if(search && document.activeElement!==search && search.value!==agentSearchValue) search.value=agentSearchValue;
...
const agentDetailScript = `
function showAgentDetail(id){
  const a=agentById(id);if(!a)return;
  const workflowRuns=DB.runs.filter(r=>DB.workflows.some(w=>w.id===r.workflow&&w.agents.includes(id)));
  document.getElementById('agentDetailBody').innerHTML=\`
    <div class="detail-header">
      <div class="detail-avatar" style="background:\${agentColor(a.role)}">\${iconMark(a.emoji)}</div>
      <div class="detail-meta">
      ...
      <div class="agent-detail-actions">
        <button class="btn btn-ghost btn-sm" onclick="closeModal('agentDetailModal');openAgentModal('\${id}')">Edit</button>
        <button class="btn btn-primary btn-sm" onclick="runAgent('\${id}')">Run</button>
      </div>
    </div>
    <div class="kv-grid">
      <div class="kv"><div class="kv-label">Workflow Runs</div><div class="kv-val">\${workflowRuns.length}</div></div>
      <div class="kv"><div class="kv-label">Max Tokens</div><div class="kv-val">\${a.tokens.toLocaleString()}</div></div>
      <div class="kv"><div class="kv-label">Temperature</div><div class="kv-val">\${a.temp.toFixed(1)}</div></div>
      <div class="kv"><div class="kv-label">Channel</div><div class="kv-val">\${a.channel||'None'}</div></div>
    </div>
  ...
  \`;
  const footer=document.querySelector('#agentDetailModal .modal-ft');
...
let workflowSearchValue='';
let workflowSearchTimer=null;
function filterWorkflows(value){
  workflowSearchValue=value||'';
  workflowSearchQuery=workflowSearchValue.trim().toLowerCase();
  if(workflowSearchTimer) clearTimeout(workflowSearchTimer);
  workflowSearchTimer=setTimeout(()=>{workflowSearchTimer=null;renderWorkflowsPage();},80);
}
...
function renderWorkflowsPage(){
  ...
  if(!body) return;
  if(search && document.activeElement!==search && search.value!==workflowSearchValue) search.value=workflowSearchValue;
  if(!DB.workflows.length){
...
function daysAgo(n){const d=new Date();d.setDate(d.getDate()-n);return formatRunDateTime(d)}
function minutesAgo(n){const d=new Date();d.setMinutes(d.getMinutes()-n);return formatRunDateTime(d)}
`;
...
function saveWorkflow(){
  ...
  if(editingWfId){
    ...
    const workflow={id:uid(),...data,created:formatRunDateTime(new Date())};
...
const RUN_LOG_WINDOW_MS=120000;
let streamClearedAt=0;
...
function runIsCurrent(run){
  return run?.status==='init' || run?.status==='running' || run?.status==='pending';
}
function logTimeMs(run,log){
  return log?.createdAtMs || parseTimeMs(log?.createdAt) || run?.createdAtMs || parseTimeMs(run?.createdAt) || 0;
}
function visibleRunLogsForStream(run){
  const cutoff=Math.max(Date.now()-RUN_LOG_WINDOW_MS,streamClearedAt);
  return terminalLogsForRun(run).filter(log=>runIsCurrent(run) || logTimeMs(run,log)>=cutoff);
}
function visibleRunsForStream(){
  return DB.runs.filter(run=>runIsCurrent(run) || visibleRunLogsForStream(run).length>0);
}
function terminalClass(type){
  return {info:'t-info',tool:'t-tool',ok:'t-ok',err:'t-err',sub:'t-sub'}[type]||'t-dim';
}
function logValueText(value){
  if(value==null) return '';
  if(typeof value==='string') return value;
  try{return JSON.stringify(value);}catch{return String(value);}
}
function normalizeLogText(value){
  return logValueText(value).replace(/\s+/g,' ').trim();
}
function logEventType(log){
  return normalizeLogText(log?.eventType||log?.type).toLowerCase();
}
function logMetadata(log){
  return log?.metadata && typeof log.metadata==='object' ? log.metadata : {};
}
function isPlaceholderLogMessage(eventType,msg,log){
  const rawType=normalizeLogText(logMetadata(log).raw_type).toLowerCase();
  const normalized=normalizeLogText(msg).toLowerCase();
  if(!normalized) return true;
  const variants=[eventType,rawType,eventType.replace(/_/g,' '),rawType.replace(/_/g,' ')].filter(Boolean);
  return variants.includes(normalized);
}
function hasMeaningfulToolSignal(log,msg){
  const metadata=logMetadata(log);
  const toolName=normalizeLogText(metadata.tool_name||metadata.name);
  const toolResult=normalizeLogText(metadata.tool_result||metadata.result||metadata.output);
  const toolInput=normalizeLogText(metadata.tool_input||metadata.input);
  return !!(toolResult || toolName || toolInput || !isPlaceholderLogMessage(logEventType(log),msg,log));
}
function displayLogMessage(log){
  const eventType=logEventType(log);
  const msg=normalizeLogText(log?.msg);
  if(eventType.includes('tool')){
    ...
    if(toolResult) parts.push('Result: '+toolResult);
    if(!isPlaceholderLogMessage(eventType,msg,log)) parts.push(msg);
    if(parts.length) return parts.join(' · ');
  }
  return msg;
}
function shouldShowRunLog(run,log){
  const eventType=logEventType(log);
  const msg=normalizeLogText(log?.msg);
  if(!msg) return false;
  if(eventType==='model_stream' || eventType==='model_metadata' || eventType.includes('delta')) return false;
  const placeholder=isPlaceholderLogMessage(eventType,msg,log);
  if(['run_queued','run_started','agent_started','run_created'].includes(eventType) && (placeholder || msg===normalizeLogText(run?.msg))) return false;
  if(eventType==='runtime_event' && placeholder) return false;
  if(eventType.includes('tool') && !hasMeaningfulToolSignal(log,msg)) return false;
  return true;
}
function displayLogsForRun(run){
  const seen=new Set();
  const logs=(run.logs||[]).filter(log=>{
    if(!shouldShowRunLog(run,log)) return false;
    const key=[logEventType(log),normalizeLogText(log.agent),normalizeLogText(log.msg),normalizeLogText(logMetadata(log).tool_name)].join('|');
...
function terminalLogsForRun(run){
  return displayLogsForRun(run);
}
function renderLogLine(run,log){
  const timestamp=log.created || formatRunDateTime(log.createdAt || new Date());
  const agent=log.agent||'runtime';
  const msg=displayLogMessage(log)||run.status||'event';
  const meta=[run.id?run.id.slice(0,8):'',log.eventType||log.type||'',log.tokens?log.tokens+' tok':'',log.cost?('$'+Number(log.cost).toFixed(4)):''].filter(Boolean).join(' · ');
  return '<div class="run-log-line">'
    + '<span class="t-ts">'+timestamp+'</span>'
    + '<span class="'+terminalClass(log.type)+'">['+agent+']</span>'
    + '<span class="run-log-msg">'+msg+'</span>'
    + '<span class="t-dim run-log-meta">'+meta+'</span>'
    + '</div>';
}
function renderTerminalFromLogs(){
  const el=document.getElementById('term-out');
  if(!el) return;
  const lines=visibleRunsForStream()
    .flatMap(run=>visibleRunLogsForStream(run).map(log=>({run,log})))
    .slice(-160);
  if(!lines.length){
    el.innerHTML='<div class="run-log-line"><span class="t-ts">--</span><span class="t-dim">[runtime]</span><span>Showing current runs and events from the last 2 minutes. Start a workflow run to stream logs here.</span></div>';
    return;
  }
  el.innerHTML=lines.map(({run,log})=>renderLogLine(run,log)).join('');
  el.scrollTop=el.scrollHeight;
}
function clearRunStream(){
  streamClearedAt=Date.now();
...
function startTerminal(){
  ...
  termInterval=setInterval(()=>{
    if(currentPage()==='runs' && runFilter==='stream'){
      renderTerminalFromLogs();
      renderRunStreamLogs();
    }
  },10000);
...
const runStreamScript = `
function renderRunStreamLogs(){
  const el=document.getElementById('run-stream-logs');
  if(!el) return;
  const runs=visibleRunsForStream();
  if(!runs.length){
    el.innerHTML='<div class="empty"><div class="empty-title">No current run logs</div><div class="empty-sub">This view only shows active runs and events from the last 2 minutes.</div></div>';
    return;
  }
  el.innerHTML=runs.map(r=>{
    const wf=wfById(r.workflow);
    const logs=visibleRunLogsForStream(r);
    const preview=logs.slice(-6);
    const eventRows=preview.length
      ? preview.map(log=>renderLogLine(r,log)).join('')
      : '<div class="run-log-line"><span class="t-ts">--</span><span class="t-dim">[runtime]</span><span class="run-log-msg">Waiting for persisted or live events...</span></div>';
...
function renderRunsTable(){
  ...
  if(!tbody) return;
  if(!filtered.length){
    tbody.innerHTML='<tr><td colspan="8"><div class="empty"><div class="empty-title">No runs found</div></div></td></tr>';
    return;
  }
  tbody.innerHTML=filtered.map(r=>{
    const wf=wfById(r.workflow);
    const agent=agentById(r.agent);
    const target=agent?iconMark(agent.emoji)+' '+agent.name:(wf?iconMark(wf.icon)+' '+wf.name:'—');
    const cancelAction=isCancelableRun(r)
      ? '<button class="btn btn-danger btn-sm" onclick="event.stopPropagation();cancelRun(\\''+r.id+'\\')">Cancel</button>'
      : '';
    return '<tr onclick="showRunDetail(\\''+r.id+'\\')">'
      ...
      + '<td class="run-message-cell">'+(r.msg||'—')+'</td>'
      + '<td><div class="run-row-actions"><button class="btn btn-ghost btn-sm" onclick="event.stopPropagation();showRunDetail(\\''+r.id+'\\')">Logs</button>'+cancelAction+'</div></td>'
      + '</tr>';
  }).join('');
}
`;
...
function apiAgentToUi(agent){
  ...
  return {
    ...
    status:'active',
    runs:0,
    created:formatRunDateTime(agent.created_at),
...
function apiWorkflowToUi(workflow){
  ...
  return {
    ...
    notes:meta.notes||'',
    created:formatRunDateTime(workflow.created_at),
...
function apiRunToUi(run){
  ...
  return {
    ...
    agent:run.agent_id||'',
    runType:run.agent_id?'agent':'workflow',
    status:run.status,
...
function apiMessageToUi(message){
  return {
    ...
    created:formatRunDateTime(message.created_at),
  };
}
function apiEventToLog(event){
  ...
  return {
    type:eventType==='run_failed'?'err':eventType==='run_cancelled'?'sub':eventType==='run_completed'||eventType==='agent_completed'?'ok':eventType.includes('tool')?'tool':'info',
    ...
    created:formatRunDateTime(createdAt),
    createdAt,
    createdAtMs:parseTimeMs(createdAt),
  };
}
function logKey(log){
  return log.eventId || [log.eventType||log.type||'',log.agent||'',log.msg||'',log.created||''].join('|');
}
function mergeRunLogs(run,logs){
  const existing=new Map((run.logs||[]).map(log=>[logKey(log),log]));
  logs.forEach(log=>existing.set(logKey(log),log));
  run.logs=[...existing.values()];
  return run.logs;
}
async function hydrateRunEvents(run){
  if(!run?.id) return [];
  const events=await apiFetch('/runs/'+run.id+'/events');
  const logs=events.map(apiEventToLog);
  mergeRunLogs(run,logs);
  run.tokens=events.reduce((sum,event)=>sum+(event.tokens_used||0),0) || run.tokens || 0;
  run.cost=events.reduce((sum,event)=>sum+(event.cost_usd||0),0) || run.cost || 0;
  return logs;
}
async function hydrateRecentRunEvents(limit=12){
  const runs=DB.runs.slice(0,limit);
  await Promise.all(runs.map(run=>hydrateRunEvents(run).catch(()=>[])));
}
...
function agentPayloadFromForm(){
  ...
  const model=document.getElementById('f-agent-model').value||'gpt-4o-mini';
  return {
...
async function showRunDetail(id){
  ...
  const displayLogs=displayLogsForRun(r);
  const logHtml=displayLogs.length
    ? displayLogs.map(l=>renderLogLine(r,l)).join('')
    : '<div class="run-log-line"><span class="t-dim">No useful persisted events yet.</span></div>';
  document.getElementById('runDetailBody').innerHTML=\`
    <div class="kv-grid">
      <div class="kv"><div class="kv-label">Run ID</div><div class="kv-val mono" style="font-size:12px">\${r.id}</div></div>
  ...
  \`;
  const footer=document.querySelector('#runDetailModal .modal-ft');
  if(footer){
    footer.innerHTML='<button class="btn btn-ghost" onclick="closeModal(\\'runDetailModal\\')">Close</button>'
      +(isCancelableRun(r)?'<button class="btn btn-danger" onclick="cancelRun(\\''+r.id+'\\')">Cancel Run</button>':'');
  }
  openModal('runDetailModal');
}
async function cancelRun(id){
  const run=DB.runs.find(r=>r.id===id);
  if(!run){notify('Run not found',false);return;}
  if(!isCancelableRun(run)){notify('Run is already '+run.status,false);return;}
  try{
    const cancelled=await apiFetch('/runs/'+id+'/cancel',{method:'POST'});
    run.status=cancelled.status||'cancelled';
    run.output=cancelled.output||run.output||'Run cancelled by user';
    const createdAt=new Date().toISOString();
    mergeRunLogs(run,[{type:'sub',agent:'runtime',msg:run.output,eventType:'run_cancelled',created:formatRunDateTime(createdAt),createdAt,createdAtMs:Date.now()}]);
    notify('Run cancelled');
    await hydrateRunEvents(run).catch(()=>[]);
    renderRunsTable();
    renderRunStreamLogs();
    renderTerminalFromLogs();
    if(document.getElementById('runDetailModal')?.classList.contains('show')) showRunDetail(id);
  }catch(error){
...
async function runAgent(agentId){
  const agent=agentById(agentId);
  if(!agent){notify('Agent not found',false);return;}
  const title=document.getElementById('agentRunModalTitle');
  const idField=document.getElementById('f-agent-run-id');
  const msgField=document.getElementById('f-agent-run-msg');
  if(title) title.textContent='Run '+agent.name;
  if(idField) idField.value=agentId;
  if(msgField) msgField.value='';
  closeModal('agentDetailModal');
  openModal('agentRunModal');
  setTimeout(()=>msgField?.focus(),0);
}
async function submitAgentRun(){
  const agentId=document.getElementById('f-agent-run-id')?.value||'';
  const input=document.getElementById('f-agent-run-msg')?.value||'';
  const agent=agentById(agentId);
  if(!agent){notify('Agent not found',false);return;}
  try{
    if(!input.trim()){notify('Input is required',false);return;}
    const run=await apiFetch('/agents/'+agentId+'/runs',{method:'POST',body:JSON.stringify({input:input.trim(),execute:true})});
    closeModal('agentRunModal');
    closeModal('agentDetailModal');
    await loadBackendData();
    notify('Agent run started in backend ✓');
...
function subscribeRunEvents(runId){
  if(!runId) return;
  const currentSocket=window.__nxflowRunSocket;
  if(currentSocket?.readyState===WebSocket.OPEN && window.__nxflowRunSocketRunId===runId) return;
  ...
  try{
    ...
    window.__nxflowRunSocketRunId=runId;
    socket.onmessage=(event)=>{
      try{
        ...
        if(run){
          ...
          mergeRunLogs(run,[{type:eventType==='run_failed'?'err':eventType==='run_completed'||eventType==='agent_completed'?'ok':eventType.includes('tool')?'tool':'info',agent:payload.agent_name||'runtime',msg:message,metadata:payload.metadata||{},eventType,eventId:payload.event_id,tokens:payload.tokens_used||0,cost:payload.cost_usd||0,created:formatRunDateTime(createdAt),createdAt,createdAtMs:parseTimeMs(createdAt)||Date.now()}]);
          if(eventType==='run_completed'||eventType==='run_failed'||eventType==='run_cancelled') loadBackendData();
    ...
    socket.onclose=()=>{
      if(window.__nxflowRunSocket===socket){
        window.__nxflowRunSocket=null;
        window.__nxflowRunSocketRunId='';
      }
    };
...
export default function PlatformConsole({ initialPage = "dashboard" }: { initialPage?: ConsolePage }) {
  ...
  useEffect(() => {
    ...
    const agentOpenAIGroup = Array.from(agentModelSelect?.querySelectorAll<HTMLOptGroupElement>("optgroup") ?? [])
      .find((group) => group.label === "OpenAI");
    if (agentModelSelect && agentOpenAIGroup && agentModelSelect.firstElementChild !== agentOpenAIGroup) {
      agentModelSelect.insertBefore(agentOpenAIGroup, agentModelSelect.firstElementChild);
      agentModelSelect.value = "gpt-4o-mini";
    }
    ...
    const consoleScript = inlineScript
      .replace(
        "const TOOL_REGISTRY = [",
      ...
      .replace("vs last week", "week over week")
      .replace("${DB.agents.length} total configured", "${DB.agents.length} total")
      .replace("p95: 3.2s · p99: 8.1s", "95% requests < 3.2s")
      .replace("~${cost} estimated cost", "~$$$$${cost} cost")
      .replace(
        /function daysAgo\(n\)\{const d=new Date\(\);d\.setDate\(d\.getDate\(\)-n\);return d\.toLocaleDateString\('en-US',\{month:'short',day:'numeric'\}\)\}\nfunction minutesAgo\(n\)\{if\(n<1\)return 'just now';if\(n<60\)return n\+'m ago';return Math\.floor\(n\/60\)\+'h ago'\}/,
        timestampHelpersScript.trim(),
      )
      .replace("started:createdAt?new Date(createdAt).toLocaleString():'—',", "started:formatRunDateTime(createdAt),")
      .replace("tokens:0,duration:'—',started:'just now',", "tokens:0,duration:'—',started:formatRunDateTime(new Date()),")
      .replace(/function renderAgentsGrid\(\)\{.*?\n\}\n\nfunction openAgentModal/s, () => `${agentsRenderScript.trim()}\n\nfunction openAgentModal`)
      .replace(/function showAgentDetail\(id\)\{.*?\n\}\n\nfunction deleteAgent/s, () => `${agentDetailScript.trim()}\n\nfunction deleteAgent`)
      ...
      .replace(/function renderRunsTable\(\)\{.*?\n\}\n\nfunction clearRunStream/s, () => `${runsTableScript.trim()}\n\nfunction clearRunStream`)
      .replace(/function renderRunStreamLogs\(\)\{.*?\n\}\n\nfunction filterRuns/s, () => `${runStreamScript.trim()}\n\nfunction filterRuns`)
      .replace(
        "const w=DB.workflows[0];",
    ...
    scriptTag.text = scopedConsoleScript(consoleScript);
    ...
    return () => {
      ...
      (window as typeof window & { __nxflowCleanup?: () => void }).__nxflowCleanup?.();
      scriptTag.remove();
