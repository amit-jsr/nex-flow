        ...
        <div class="credential-grid">
          <div class="credential-card">
            <div class="credential-card-top">
              <span class="badge b-purple">Groq</span>
            </div>
            <div class="credential-card-fields">
              <div class="form-group">
                <label class="form-label">Groq API key</label>
                <input class="form-input" type="password" id="s-groq" aria-label="Groq API key" placeholder="Groq API key · gsk...">
              </div>
              <input type="hidden" id="s-groq-base" value="https://api.groq.com/openai/v1">
            </div>
          </div>
      ...
      <div class="settings-panel-body">
        <div class="model-defaults-row">
          ...
          <div class="form-group">
            <label class="form-label">Default Model</label>
            <select class="form-input form-select" id="s-default-model">
              <optgroup label="Groq" data-provider="Groq"><option selected>openai/gpt-oss-20b</option><option>llama-3.3-70b-versatile</option><option>openai/gpt-oss-120b</option><option>groq/compound</option><option>groq/compound-mini</option><option>llama-3.1-8b-instant</option></optgroup>
...
async function syncSavedSettingsToBackend({silent=false}={}){
  let saved={};
  try{saved=JSON.parse(localStorage.getItem('nxflow.settings')||'{}');}catch{}
  const payload=runtimeProviderSettings(saved);
  if(!payload.groq_api_key&&!payload.openai_api_key&&!payload.anthropic_api_key) return false;
  try{
    await apiFetch('/settings/',{
      method:'PATCH',
      body:JSON.stringify(payload)
    });
    if(!silent) notify('Provider keys synced to backend ✓');
    return true;
  }catch(error){
    if(!silent) notify('Could not sync provider keys to backend: '+error.message,false);
    return false;
  }
}
function runtimeProviderSettings(saved={}){
  return {
    groq_api_key:saved.groqKey||saved.grokKey||document.getElementById('s-groq')?.value||'',
    groq_base_url:saved.groqBaseUrl||saved.grokBaseUrl||document.getElementById('s-groq-base')?.value||'https://api.groq.com/openai/v1',
    groq_default_model:saved.defaultModel||document.getElementById('s-default-model')?.value||'openai/gpt-oss-20b',
    openai_api_key:saved.openaiKey||document.getElementById('s-openai')?.value||'',
    anthropic_api_key:saved.anthropicKey||document.getElementById('s-anthropic')?.value||'',
  };
}
...
async function triggerRun(){
  ...
  if(!msg){notify('Input message is required',false);return;}
  try{
    await syncSavedSettingsToBackend({silent:true});
    let savedSettings={};
    try{savedSettings=JSON.parse(localStorage.getItem('nxflow.settings')||'{}');}catch{}
    const run=await apiFetch('/workflows/'+wfId+'/runs',{method:'POST',body:JSON.stringify({input:msg,execute:true,provider_settings:runtimeProviderSettings(savedSettings)})});
    closeModal('runModal');
    await loadBackendData();
...
syncSavedSettingsToBackend({silent:true});
loadBackendData();
`;
...
export default function PlatformConsole({ initialPage = "dashboard" }: { initialPage?: ConsolePage }) {
  ...
  useEffect(() => {
    ...
    const applySavedSettings = () => {
      ...
      setFieldValue("s-groq", saved.groqKey ?? saved.grokKey);
      setFieldValue("s-groq-base", saved.groqBaseUrl ?? saved.grokBaseUrl);
      const savedProvider = typeof saved.defaultProvider === "string" ? saved.defaultProvider : "";
      const normalizedProvider = savedProvider.toLowerCase() === "groq" || savedProvider.toLowerCase() === "grok"
        ? "Groq"
        : savedProvider;
      setFieldValue("s-default-provider", normalizedProvider);
  ...
  const valid={OpenAI:model.startsWith('gpt-'),Anthropic:model.startsWith('claude-'),Groq:model.startsWith('groq/')||model.startsWith('llama-')||model.startsWith('openai/')}[provider];
  ...
  const savedSettings={
    ...
    groqKey:document.getElementById('s-groq')?.value||'',
    groqBaseUrl:document.getElementById('s-groq-base')?.value||'',
  ...
  try{
    await apiFetch('/settings/',{
      method:'PATCH',
      body:JSON.stringify({
        groq_api_key:savedSettings.groqKey,
        groq_base_url:savedSettings.groqBaseUrl,
        groq_default_model:savedSettings.defaultModel,
        openai_api_key:savedSettings.openaiKey,
        anthropic_api_key:savedSettings.anthropicKey,
      })
    });
    notify('Settings saved to backend ✓');
  }catch(error){
    notify('Saved locally, but backend settings update failed: '+error.message,false);
  }
}`,
      )
      .replace(
