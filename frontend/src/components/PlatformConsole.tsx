...
function terminalLogsForRun(run){
  const logs=(run.logs||[]).filter(log=>log.msg);
...
function renderLogLine(run,log){
  const timestamp=log.created || (log.createdAt ? new Date(log.createdAt).toLocaleTimeString() : new Date().toLocaleTimeString());
...
function renderRunsTable(){
  ...
  tbody.innerHTML=filtered.map(r=>{
    ...
    return '<tr onclick="showRunDetail(\\''+r.id+'\\')">'
      ...
      + '<td class="mono">'+r.duration+'</td>'
      + '<td style="color:var(--muted)">'+r.started+'</td>'
      + '<td><div class="run-row-actions"><button class="btn btn-ghost btn-sm" onclick="event.stopPropagation();showRunDetail(\\''+r.id+'\\')">View logs</button>'+cancelAction+'</div></td>'
      + '</tr>';
  }).join('');
}
`;
...
function apiRunToUi(run){
  ...
  return {
    ...
    started:createdAt?new Date(createdAt).toLocaleString():'—',
    createdAt,
...
async function showRunDetail(id){
  ...
  const logHtml=logs.length
    ? logs.map(l=>'<div class="run-log-line"><span class="t-ts">'+(l.created||'——')+'</span><span class="'+({info:'t-info',tool:'t-tool',ok:'t-ok',err:'t-err',sub:'t-sub'}[l.type]||'t-dim')+'">['+l.agent+']</span><span class="run-log-msg">'+l.msg+'</span><span class="t-dim run-log-meta">'+[l.eventType||l.type||'',l.tokens?l.tokens+' tok':'',l.cost?('$'+Number(l.cost).toFixed(4)):''].filter(Boolean).join(' · ')+'</span></div>').join('')
...
async function cancelRun(id){
  ...
  try{
    ...
    mergeRunLogs(run,[{type:'sub',agent:'runtime',msg:run.output,eventType:'run_cancelled',created:new Date(createdAt).toLocaleTimeString(),createdAt,createdAtMs:Date.now()}]);
...
async function submitAgentRun(){
  ...
  if(!agent){notify('Agent not found',false);return;}
  try{
    if(!input.trim()){notify('Input message is required',false);return;}
    ...
    socket.onmessage=(event)=>{
      try{
        ...
        if(run){
          ...
          const createdAt=new Date().toISOString();
          mergeRunLogs(run,[{type:eventType==='run_failed'?'err':eventType==='run_completed'||eventType==='agent_completed'?'ok':eventType.includes('tool')?'tool':'info',agent:payload.agent_name||'runtime',msg:message,eventType,tokens:payload.tokens_used||0,cost:payload.cost_usd||0,created:new Date(createdAt).toLocaleTimeString(),createdAt,createdAtMs:Date.now()}]);
