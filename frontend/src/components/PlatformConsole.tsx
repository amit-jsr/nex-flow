  ...
  <div class="settings-grid">
    ...
    <section class="panel settings-storage">
      ...
      <div class="settings-panel-body">
        <div class="form-group">
          <label class="form-label">PostgreSQL URL</label>
          <input class="form-input" id="s-pg" value="postgresql://nexflow:nexflow@localhost/nexflow">
        </div>
...
const API_BASE=localStorage.getItem('nexflow.apiBase')||'http://localhost:8000';
...
function subscribeRunEvents(runId){
  if(!runId||window.__nexflowRunSocket?.readyState===WebSocket.OPEN) return;
  const wsBase=API_BASE.replace(/^http/,'ws');
  try{
    const socket=new WebSocket(wsBase+'/ws/runs/'+runId);
    window.__nexflowRunSocket=socket;
...
window.nexflowRefresh=loadBackendData;
loadBackendData();
`;
...
export default function PlatformConsole({ initialPage = "dashboard" }: { initialPage?: ConsolePage }) {
  ...
  useEffect(() => {
    ...
    const readSavedSettings = () => {
      try {
        return JSON.parse(localStorage.getItem("nexflow.settings") ?? "{}") as Record<string, unknown>;
      } catch {
        return {};
      }
    };
    ...
    scriptTag.text = inlineScript
      .replace(
        "{id:'calculator',name:'Calculator',source:'Built-in',category:'Compute',provider:'NexFlow'",
        "{id:'calculator',name:'Calculator',source:'Strands',category:'Compute',provider:'Runtime'",
      )
      .replace(
        "{id:'current_time',name:'Current Time',source:'Built-in',category:'Utility',provider:'NexFlow'",
        "{id:'current_time',name:'Current Time',source:'Strands',category:'Utility',provider:'Runtime'",
      )
      .replace(
        "{id:'http_batch_request',name:'HTTP Batch Request',source:'Built-in',category:'Actions',provider:'NexFlow'",
        "{id:'http_batch_request',name:'HTTP Batch Request',source:'Strands',category:'Actions',provider:'Runtime'",
      )
      .replace(
        "{id:'memory_lookup',name:'Memory Lookup',source:'Built-in',category:'Memory',provider:'NexFlow'",
      ...
      .replace(
        "return {'Built-in':'NexFlow platform tools', Integration:'Provider-backed tools', MCP:'Tools exposed by MCP servers'}[source]||'Tools';",
        "return {Strands:'Runtime-native tools', 'Built-in':'NexFlow platform tools', Integration:'Provider-backed tools', MCP:'Tools exposed by MCP servers'}[source]||'Tools';",
      )
      .replace(
        "return {'Built-in':'b-blue',Integration:'b-amber',MCP:'b-green'}[source]||'b-muted';",
  ...
  localStorage.setItem('nexflow.settings',JSON.stringify({
